# this function will end up as the workhorse of splithalf, this way people only have to call one function
...
# to change - change iterations to permutations


# you could make a little tutorial about trimming the data to acompany the doc
## e.g. with min and max trims, a quick sd cutoff
## removing errors etc


# add that this requires the robustbase package


# unofficial function version name: "This function gives me the power to fight like a crow"

splithalf_master <- function(data,
                             outcome = "RT",
                             score = "difference",
                             conditionlist = FALSE,
                             halftype = "random",
  ...
  if (halftype == "oddeven" | halftype == "halfs")
  {
    ...
    if(score == "difference_of_difference") {
      splithalf <- finaldata2 %>%
        ...
        tidyr::unite(compare, j, bias, sep = "_") %>%
